...
if __name__=='__main__':
    while(1):
        FormatHour = '%H:%M'
        HOUR = time.strftime(FormatHour)
        FormatMin = '%M'
        MIN = time.strftime(FormatMin)
        FormatSec = '%S'
        SEC = time.strftime(FormatSec)
        if SEC == '00':
            ...
            write_csv(["[h3][b]Récompenses du "+str(old_date)+" au "+str(current_date)+"[/b][/h3]"],csv)
            print('')
            write_csv([''],csv)
            ...
            write_csv(["Comme chaque semaine voici le classement des votes sur nos différents top-sites. Pour rappel, si vous franchissez la barre des [b]50/100/150 votes[/b], vous pouvez gagner jusqu'à [b]3/6/9 de Renommée[/b]."],csv)
            print('')
            write_csv([''],csv)
            print("Les récompensés sont donc les suivants :[list=1]")
            write_csv(["Les récompensés sont donc les suivants :[list=1]"],csv)
            i = 0
            for x,y in zip(Name,XP):
                ...
                if b > 0:
                    ...
                    write_csv([change(x,LISTE_MEMBRES_GOTEI_13,LISTE_MEMBRES_ACUERDO,LISTE_MEMBRES_ULTIMA_NECAT,LISTE_MEMBRES_INDEP)+" : "+ str(y)+ " Renommée"],csv)
                i += 1
            ...
            write_csv(["BTC compte " + str(len(Votes)) +" votants pour cette session."],csv)
            print("")
            write_csv([''],csv)
            print("[h2]Merci et bon jeu sur BTC ![/h2]")
            write_csv(["[h2]Merci et bon jeu sur BTC ![/h2]"],csv)
